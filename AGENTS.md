...
Current content shape: 293 Markdown files — 185 chapters (Part I 33, II 19, III 12, IV 12, V 14,
VI 16, VII 36, VIII 17, IX 26), 8 `00-essentials` reference pages, 53 `_index.md` navigation pages, `content/about.md`,
`content/guide.md`, `content/table-of-contents.md`, `content/terms-of-use.md`, and `content/_index.md`. All 185 chapters open with the §5 sections in that order.
...
Every content file starts with **YAML** front matter delimited by `---`. All 293 files use YAML;
never use TOML `+++`.
...
- Every language implements the **same operation set and the same algorithm** — no language gets a
  simplified version.
- **Compliance today: complete.** All 33 Part I chapters carry one contiguous six-language group, and
...
- Run `python tools/check_links.py` after any link or file move; it also reports missing front
  ...
  (Current state: 293 files checked, 0 problems.)
